...
def validation(net, val_data_loader, device, exp_name, save_tag=False, if_clamp=False):
...
def validation_val(net, val_data_loader, device, exp_name, save_tag=False, if_clamp=True):

    save_dir = os.path.join(exp_name, "imgs")
    if save_tag:
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        ...
        with torch.no_grad():
            ...
            pred_image = net(input_im)
            infer_speed = time.time() - start_time
            infer_time.append(infer_speed)
            if if_clamp:
                pred_image = torch.clamp(pred_image, 0., 1.)

            if save_tag:
                base = os.path.basename(imgid[0])
                utils.save_image(input_im[0], os.path.join(save_dir, base + "_input.png"), normalize=True)
                utils.save_image(gt[0], os.path.join(save_dir, base + "_gt.png"), normalize=True)
                utils.save_image(pred_image[0], os.path.join(save_dir, base + "_pred.png"), normalize=True)
    ...
    avr_infer_time = sum(infer_time) / len(infer_time)
    print("====> [INFO] average inference time: ", avr_infer_time)
    return avr_psnr, avr_ssim
...
def adjust_learning_rate(optimizer, epoch,  lr_decay=0.5, step=10000):
